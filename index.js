    ...
    // TODO: check for no results
		...
		if (elem.length > 0) {
			...
			// send no results message
            		sendMessage(recipientId, {text: 
	"No results found. Make sure your category input isn't misspelled and that the category exists!"
				});
			called = false;
			return true;			
		}
		...
		if (elem.length > 0) {
			...
			// send no results message
            		sendMessage(recipientId, {text: 
	"No results found. Make sure your search terms aren't misspelled and that the song exists!"
				});
			called = false;
			return true;			
		}
